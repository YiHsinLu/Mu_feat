...
Xtrain = as.data.frame(cbind(scale(mu_feat[sel,1:28]), mu_feat[sel, 29]))
Xtest = as.data.frame(cbind(scale(mu_feat[-sel,1:28]), mu_feat[-sel, 29]))

colnames(Xtrain)=colnames(Xtest)=colnames(mu_feat)
...
svm_std = svm(as.factor(label) ~., data = Xtrain, scale = T)

summary(svm_std)
```

```{r}
y_pre = predict(svm_std, Xtest[,-29])
y = as.factor(Xtest[,29])
